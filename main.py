...
i = 0
j = 0
DailyD = StockData.findall('Daily')
length = len(DailyD)

for tr in tab[1].findAll('tr'):  #tab[1] = 三大法人
    for td in tr.findAll('td'):
        if i >= 2:
            DiffAll = DailyD[length+1-i].find('Difference')
            if j % 4 == 2:  # 投信
                DiffAll.set('InvestmentTrustDiff',(td.getText().rstrip()).replace(',',"") )
            if j % 4 == 3:  # 自營商
                DiffAll.set('DealersDiff',(td.getText().rstrip()).replace(',',""))
            if j % 4 == 0:  # 外資
                DiffAll.set('ForeignInvestorsDiff',(td.getText().rstrip()).replace(',',"") )
        j += 1
    i += 1
...
for tr in tab[2].findAll('tr'):  #tab[2] = 主力庫存
    for td in tr.findAll('td'):
        if i >= 2:
            DiffAll = DailyD[length+1-i].find('Difference')
            if j % 3 == 0:  # 主力增減
                DiffAll.set('LeaderDiff',(td.getText().rstrip()).replace(',',"") )
        j += 1
    i += 1
